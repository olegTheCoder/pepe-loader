...
import type { PepeLoaderProps } from '../types';
import { lighten } from '../utils/color';

export const PepeLoader: React.FC<PepeLoaderProps> = ({
  size = 300,
  speed = 1,
  texts = ['ФААА', 'ШНЕЛЕ', 'ПЭПЭ'],
  colors = ['#FF0000', '#FFD700', '#00BFFF'],
  textColor = '#000000',
  fontSizes = [38, 32, 36],
  fontWeight = 900,
  ...
  const [fs1, fs2, fs3] = fontSizes;

  const s1a = c1;
  const s1b = lighten(c1, 0.27);
  const s1c = c1;
  const s1d = lighten(c1, 0.4);

  const s2a = c2;
  const s2b = lighten(c2, 0.2);
  const s2c = c2;

  const s3a = c3;
  const s3b = lighten(c3, 0.3);
  const s3c = c3;
  ...
  return (
    ...
    >
      ...
      <g className="b1">
        <path fill="none" stroke={c1} strokeWidth={burstStrokeWidth} strokeLinejoin="round" strokeLinecap="round"
          d="M 100 40 L 115 80 L 150 75 L 130 105 L 160 135 L 120 125 L 100 160 L 80 125 L 40 135 L 70 105 L 50 75 L 85 80 Z" />
      </g>
      <circle className="s1a" cx="100" cy="80" r="3" fill={s1a} />
      <circle className="s1b" cx="100" cy="80" r="2.5" fill={s1b} />
      <circle className="s1c" cx="100" cy="120" r="3" fill={s1c} />
      <circle className="s1d" cx="100" cy="120" r="2" fill={s1d} />
      <text x="100" y="105" className="t1"
        fontFamily={fontFamily}
        fontSize={fs1}
        fontWeight={fontWeight}
        textAnchor="middle" dominantBaseline="middle"
        fill={textColor} stroke="none">{t1}</text>

      <g className="b2">
        <path fill="none" stroke={c2} strokeWidth={burstStrokeWidth} strokeLinejoin="round" strokeLinecap="round"
          d="M 100 20 L 120 80 L 180 100 L 120 120 L 100 180 L 80 120 L 20 100 L 80 80 Z" />
      </g>
      <circle className="s2a" cx="100" cy="80" r="3" fill={s2a} />
      <circle className="s2b" cx="100" cy="80" r="2.5" fill={s2b} />
      <circle className="s2c" cx="100" cy="130" r="3" fill={s2c} />
      <text x="100" y="105" className="t2"
        fontFamily={fontFamily}
        fontSize={fs2}
        fontWeight={fontWeight}
        textAnchor="middle" dominantBaseline="middle"
        fill={textColor} stroke="none">{t2}</text>

      <g className="b3">
        <path fill="none" stroke={c3} strokeWidth={burstStrokeWidth} strokeLinejoin="round" strokeLinecap="round"
          d="M 80 70 A 30 30 0 0 1 120 70 A 30 30 0 0 1 160 110 A 30 30 0 0 1 120 150 A 30 30 0 0 1 80 110 A 30 30 0 0 1 80 70 Z" />
      </g>
      <circle className="s3a" cx="100" cy="80" r="3" fill={s3a} />
      <circle className="s3b" cx="100" cy="80" r="2.5" fill={s3b} />
      <circle className="s3c" cx="100" cy="120" r="2.5" fill={s3c} />
      <text x="100" y="105" className="t3"
        fontFamily={fontFamily}
        fontSize={fs3}
        fontWeight={fontWeight}
        textAnchor="middle" dominantBaseline="middle"
        fill={textColor} stroke="none">{t3}</text>
    </svg>
  );
};
